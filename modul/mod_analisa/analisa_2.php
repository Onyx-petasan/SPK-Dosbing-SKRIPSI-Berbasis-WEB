...
?>
 ...
 $cek=mysql_query("SELECT * FROM tugasakhir WHERE id_tugasakhir='$_GET[id]'");
  ...
  mysql_query("UPDATE detail_dosen SET nama   = '$_POST[nama]',
                                  email          = '$_POST[email]',
								  nip          = '$_POST[nip]',								  
...
$carimax3 = mysql_query("SELECT max(kompetensi.nilaii) as max3
								FROM kompetensi,dosen WHERE kompetensi.id_dosen=dosen.id_dosen AND kompetensi.id_parameter='7'");
...
<h4 class="box-title">NORMALISASI</h4>
  ...
  foreach($R as $i=>$r){ 
    for($j=0;$j<$m;$j++){ 
	  ...
	  mysql_query("UPDATE dosen SET skor   = {$P[$i]}
                           WHERE  id_dosen     = {$i}");
    } 
